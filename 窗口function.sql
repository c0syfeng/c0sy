...
SELECT *,sum(salary) over(partition by dept_no ) as st from SQL_7

SELECT *,salary/st as 占比 FROM(
          SELECT *,sum(salary) over(partition by dept_no) as st from SQL_7
					) AS temp;

问题三：对各部门员工的工资进行从小到大排序，排名前30%为低层，30%-80%为中层，高于80%为高层，并打上标签

SELECT *,ROW_NUMBER() over(partition by dept_no ORDER BY salary)FROM SQL_7

SELECT *,ROW_NUMBER() over(PARTITION by dept_no ORDER BY 占比) as 工资占比排名 from(
       SELECT *,salary/st as 占比 FROM(
              SELECT *,sum(salary) over(partition by dept_no) as st from SQL_7
					) AS temp
) as t1;
